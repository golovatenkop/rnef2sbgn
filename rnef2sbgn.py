...
def read_scene(filename, plot_scale):
    ret = {
        'glyphs': {},
        'links': {},
        'arcs': {},
...
def do_the_job(filename_in, filename_out, language, plot_scale):
    scene = read_scene(filename_in, plot_scale)
    classmap = read_classmap('classes.xml', language)
    sbgn = compile_sbgn(scene, classmap)
    xmlstr = minidom.parseString(et.tostring(sbgn)).toprettyxml(indent='    ')
    with open(filename_out, mode='w', encoding='utf8') as f:
        f.write(xmlstr)
